...
{
	[Route("/api/v1/edu/markets")]
	public class EducationMarketsController : EducationControllerBase
	{
		protected override EducationTutorial Tutorial => EducationTutorial.FinanceMarkets;

		private readonly IGrpcServiceProxy<ITutorialMarketsService> _tutorialService;

		public EducationMarketsController(IGrpcServiceProxy<ITutorialMarketsService> tutorialService,
			IEncoderDecoder encoderDecoder, ISystemClock systemClock,
			ILogger<EducationMarketsController> logger) : base(systemClock, encoderDecoder, logger) => _tutorialService = tutorialService;
