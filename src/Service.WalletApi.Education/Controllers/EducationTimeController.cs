...
{
	[Route("/api/v1/edu/time")]
	public class EducationTimeController : EducationControllerBase
	{
		protected override EducationTutorial Tutorial => EducationTutorial.TimeManagement;

		private readonly IGrpcServiceProxy<ITutorialTimeService> _tutorialService;

		public EducationTimeController(IGrpcServiceProxy<ITutorialTimeService> tutorialService,
			IEncoderDecoder encoderDecoder, ISystemClock systemClock,
			ILogger<EducationTimeController> logger) : base(systemClock, encoderDecoder, logger) => _tutorialService = tutorialService;
