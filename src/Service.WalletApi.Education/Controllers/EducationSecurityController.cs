...
{
	[Route("/api/v1/edu/security")]
	public class EducationSecurityController : EducationControllerBase
	{
		protected override EducationTutorial Tutorial => EducationTutorial.FinanceSecurity;

		private readonly IGrpcServiceProxy<ITutorialSecurityService> _tutorialService;

		public EducationSecurityController(IGrpcServiceProxy<ITutorialSecurityService> tutorialService,
			IEncoderDecoder encoderDecoder, ISystemClock systemClock,
			ILogger<EducationSecurityController> logger) : base(systemClock, encoderDecoder, logger) => _tutorialService = tutorialService;
