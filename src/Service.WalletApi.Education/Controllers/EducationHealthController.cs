...
{
	[Route("/api/v1/edu/health")]
	public class EducationHealthController : EducationControllerBase
	{
		protected override EducationTutorial Tutorial => EducationTutorial.HealthAndFinance;

		private readonly IGrpcServiceProxy<ITutorialHealthService> _tutorialService;

		public EducationHealthController(IGrpcServiceProxy<ITutorialHealthService> tutorialService,
			IEncoderDecoder encoderDecoder, ISystemClock systemClock,
			ILogger<EducationHealthController> logger) : base(systemClock, encoderDecoder, logger) => _tutorialService = tutorialService;
